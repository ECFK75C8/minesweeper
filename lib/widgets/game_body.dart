...
class GameBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Consumer<CellItems>(
        builder: (_, cellData, __) => GridView.builder(
            ...
            return InkWell(
              onTap: () {
                ...
                if (!cellData.grid[x][y].flag) {
                  if (!cellData.started) context.read<TimerItem>().startTimer();

                  if (!cellData.reveal(x, y)) context.read<TimerItem>().stop();
              ...
              child:
                  GridItem(cellData.grid, Offset(x.toDouble(), y.toDouble())),
            );
          },
        ),
      );
